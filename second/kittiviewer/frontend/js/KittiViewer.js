// Hardcoded paths.
var datasetClassName2Paths = {
    'KittiDataset': {
        'rootPath': "/home/sancha/data/kitti_detection",
        'infoPath': "/home/sancha/data/kitti_detection/kitti_infos_train.pkl",
        'configPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/cars.fhd.all_beams/pipeline_kitti.config.yaml",
        // 'configPath': "/home/sancha/repos/second.pytorch/second/configs/pointpillars/car/xyres_20.config",
        'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/cars.fhd.all_beams/voxelnet-23200.tckpt"
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/kitti/pp/xyres_20.config/voxelnet-287680.tckpt"
    },
    'VirtualKittiDataset': {
        'rootPath': '/home/sancha/data/vkitti',
        'infoPath': '/home/sancha/data/vkitti/vkitti_infos_train.pkl',
        // 'configPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/cars.fhd.all_beams/pipeline_vkitti.config.yaml",
        'configPath': '/home/sancha/repos/second.pytorch/second/configs/vkitti/second/base.yaml',
        // 'configPath': '/home/sancha/repos/second.pytorch/second/configs/vkitti/pp/base.yaml',
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/cars.fhd.all_beams/voxelnet-23200.tckpt"
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/vkitti/mini_trainset/second/lc_cum/voxelnet-290.tckpt"
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/vkitti/full_trainset/ParallelMaxEntropy/lc_cum/voxelnet-2000.tckpt"
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/vkitti/full_trainset/DPOptimizedPolicy/lc_cum/voxelnet-9465.tckpt"
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/vkitti/full_trainset/DPOptimizedPolicy/sparse_sbl_rate_0pc/voxelnet-8000.tckpt"
        'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/interpDP/vkitti/second/base/voxelnet-15775.tckpt"
        // 'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/interpDP/vkitti/pp/base/voxelnet-5700.tckpt"
    },
    'SynthiaDataset': {
        'rootPath': '/home/sancha/data/synthia',
        'infoPath': '/home/sancha/data/synthia/synthia_infos_train.pkl',
        'configPath': '/home/sancha/repos/second.pytorch/second/configs/synthia/second/base.yaml',
        'checkpointPath': "/home/sancha/repos/second.pytorch/second/sid_trained_models/interpDP/synthia/second/base/voxelnet-15775.tckpt"
    }
}

var KittiViewer = function (pointCloud, lcCloud, lcNetInput, sbCloud, logger, imageCanvas, bevHmapManager) {
...
KittiViewer.prototype = {
    ...
    load: function () {
        let self = this;
        let data = {};
        data["root_path"] = datasetClassName2Paths[this.datasetClassName]["rootPath"];
        data["info_path"] = datasetClassName2Paths[this.datasetClassName]["infoPath"];
        // data["root_path"] = this.rootPath;
        // data["info_path"] = this.infoPath;
        data["dataset_class_name"] = this.datasetClassName;

        // Set imagePanel dimensions according to datasetClassName.
        var w, h;
        if (self.datasetClassName == "VirtualKittiDataset") {
            w = 1242 / 3;
            h = 375 / 3 + 37.53;  // so hacky I should be ashamed
        } else {
            w = 640 / 2;
            h = 480 / 2 + 37.53;  // so hacky I should be ashamed
        }
        self.imagePanel.resize({width: w, height: h});
        self.imageCanvas.width = w;
        self.imageCanvas.height = h;

        self.configPath = datasetClassName2Paths[this.datasetClassName]["configPath"];
        self.checkpointPath = datasetClassName2Paths[this.datasetClassName]["checkpointPath"];
        return $.ajax({
        ...
        if (this.imageIndexes.length != 0 && this.imageIndexes.includes(image_idx)) {
            ...
            var ajax1 = $.ajax({
                ...
                success: function (response) {
                    ...
                    // --------------------------------------------------------------------------
                    // Single-beam lidar.
                    var sb_points_buf = str2buffer(atob(response["sb_pointcloud"]));
                    var sb_points;
                    if (self.enableInt16){
                    ...
                    // Remove heatmap plane when switcing to another frame.
                    self.bevHmapManager.removeHmap();

                    // Remove light curtain when switching to another frame.
                    // LC Cloud
                    self.lcCloud.geometry.setDrawRange(0, 0);
                    self.lcCloud.geometry.attributes.position.needsUpdate = true;
                    self.lcCloud.geometry.attributes.color.needsUpdate = true;
                    self.lcCloud.geometry.computeBoundingSphere();
                    self.lcCloud.visible = true;
                    // LC Net Input.
                    self.lcNetInput.geometry.setDrawRange(0, 0);
                    self.lcNetInput.geometry.attributes.position.needsUpdate = true;
                    self.lcNetInput.geometry.attributes.color.needsUpdate = true;
                    self.lcNetInput.geometry.computeBoundingSphere();
                    self.lcNetInput.visible = false;
...
var BevHmapManager = function(bevPanel, bevCanvas) {
    ...
    this.hmapMode = "Confidence";  // either "Confidence" or "Entropy"
...
BevHmapManager.prototype = {
    ...
    loadHmapTexture : function() {
        ...
        if (self.confidenceMap_b64 == null)
            throw "SID: cannot call loadHmapTexture() when self.confidenceMap_64 is null.";
