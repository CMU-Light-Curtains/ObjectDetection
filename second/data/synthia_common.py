...
IDX2METADATA = None
idx2metadata_file = pathlib.Path(__file__).resolve().parent / "SynthiaImageSets" / "idx2metadata.txt"
...
def get_image_index_str(img_idx):
    return "{:08d}".format(img_idx)


def get_synthia_info_path(folder,
                          frame_id,
                          prefix,
                          info_type='image_2',
                          file_tail='.png',
                          relative_path=True,
                          exist_check=True):
    rel_file_path = f"{folder}/{info_type}/{frame_id}{file_tail}"
    abs_file_path = pathlib.Path(prefix) / rel_file_path
...
def get_image_path(folder, frame_id, root_path, relative_path=True, exist_check=True):
    return get_synthia_info_path(folder, frame_id, root_path, 'RGB', '.png', relative_path, exist_check)

def get_label_path(folder, frame_id, root_path, relative_path=True, exist_check=True):
    return get_synthia_info_path(folder, frame_id, root_path, 'labels_kitti', '.txt', relative_path, exist_check)


def get_depth_path(folder, frame_id, root_path, relative_path=True, exist_check=True):
    return get_synthia_info_path(folder, frame_id, root_path, 'Depth', '.png', relative_path, exist_check)

def get_calib_path(folder, frame_id, root_path, relative_path=True, exist_check=True):
    return get_synthia_info_path(folder, frame_id, root_path, 'calib_kitti', '.txt', relative_path, exist_check)

def _extend_matrix(mat):
    mat = np.concatenate([mat, np.array([[0., 0., 0., 1.]])], axis=0)
    return mat

def _check_synthia_directory(root_path):
    root_path = pathlib.Path(root_path)
    results = []
    results.append((root_path / 'train').exists())
    results.append((root_path / 'test').exists())
    return np.array(results, dtype=np.bool)

def _create_idx2metadata_file():
    synthia_dir = pathlib.Path(os.environ["DATADIR"]) / "synthia"

    metadata, trainids, testids = [], [], []

    train_dir = synthia_dir / "train"
    test_dir = synthia_dir / "test"
    for ttdir in [train_dir, test_dir]:
        for dir in sorted([e for e in ttdir.iterdir() if e.is_dir()]):
            for subdir in sorted([e for e in dir.iterdir() if e.is_dir()]):
                if not (subdir / "labels_kitti").is_dir():
                    continue # empty data.
                for imfile in sorted((subdir / "labels_kitti").iterdir()):
                    # Check if there is a Car in the scene.
                    with open(imfile, 'r') as f:
                        text = f.read()
                        if 'Car' not in text:
                            continue
                    frame_id = imfile.name.split('.')[0]
                    metadata.append((subdir, frame_id))
    
    with open(idx2metadata_file, 'w') as f:
        for idx, metadatum in enumerate(metadata):
            idx = str(idx).rjust(8, '0')
            dir, frame_id = metadatum
            folder = dir.relative_to(synthia_dir)
            if str(folder).startswith('train/'):
                trainids.append(idx)
            else:
                testids.append(idx)
            print(f"{idx} {folder} {frame_id}", file=f)
    
    np.random.seed(0)

    with open("data/SynthiaImageSets/train.txt", 'w') as f:
        for idx in np.random.permutation(trainids):
            print(idx, file=f)
    
    with open("data/SynthiaImageSets/test.txt", 'w') as f:
        for idx in np.random.permutation(testids):
            print(idx, file=f)

def get_synthia_image_info(path,
                           label_info=True,
                           depth=False,
                           calib=False,
                           image_ids=7481,
    ...
    def map_func(idx):
        folder, frame_id = IDX2METADATA[idx]
        info = {}
        depth_info = {}
        calib_info = {}
        ...
        annotations = None
        if depth:
            depth_info['depth_path'] = get_depth_path(folder, frame_id, root_path, relative_path)
            info['depth'] = depth_info
        
        image_info['image_path'] = get_image_path(folder, frame_id, root_path, relative_path)
        ...
        if label_info:
            label_path = get_label_path(folder, frame_id, root_path, relative_path)
            if relative_path:
        ...
        if calib:
            calib_path = get_calib_path(folder, frame_id, root_path, relative_path=False)
...
def filter_synthia_anno(image_anno,
                       used_classes,
...
def synthia_result_line(result_dict, precision=4):
...
def annos_to_synthia_label(annos):
    ...
    for i in range(num_instance):
        ...
        line = synthia_result_line(result_dict)
...
def get_label_annos(root_path, image_ids):
    ...
    for idx in image_ids:
        folder, frame_id = IDX2METADATA[idx]
        label_path = get_label_path(folder, frame_id, root_path, relative_path=False)
